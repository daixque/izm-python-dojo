...
- 遊園地のアトラクション利用判定プログラム
  - ユーザーの年齢と身長を引数として受け取る関数 `can_ride(age, height)` を実装
  - アトラクションの条件：「6歳以上」かつ「身長120cm以上」
  - 条件を満たす場合は `True` を返し、満たさない場合は `False` を返す
  - 初期コード：関数の枠組みと、テスト用のメインプログラムは用意済み
  - ユーザーは関数内の条件判定部分（`and` を使った条件式）のみを実装
  - テスト：複数の年齢・身長の組み合わせで正しく判定できるかをチェック
    - 例：`can_ride(8, 125)` → `True`（両方の条件を満たす）
    - 例：`can_ride(5, 130)` → `False`（年齢が条件を満たさない）
    - 例：`can_ride(10, 115)` → `False`（身長が条件を満たさない）
